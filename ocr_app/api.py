...
async def perform_ocr(
    ...
    url_body: Optional[str] = Body(default=None),
    engine: str = Form(default=config.default_engine),
    engine_body: Optional[str] = Body(default=None),
    languages: Optional[List[str]] = Form(default=None),
    languages_body: Optional[List[str]] = Body(default=None),
    dpi: Optional[int] = Form(default=None),
    dpi_body: Optional[int] = Body(default=None),
):
    """Run OCR on an uploaded file or remote URL.

    The endpoint supports multipart/form-data uploads and JSON payloads with a ``url`` field.
    """

    selected_engine = engine_body or engine
    selected_languages = languages_body or languages or config.default_languages
    selected_dpi = dpi_body or dpi or config.pdf_dpi
    preprocess_options = config.preprocess_options
    url = url_body or url_form
    ...
    if file:
        ...
        return _build_response(file.filename or "upload", selected_engine, selected_languages, pages)
    ...
    try:
        ...
        raise HTTPException(status_code=400, detail=str(exc)) from exc
    return _build_response(filename, selected_engine, selected_languages, pages)
