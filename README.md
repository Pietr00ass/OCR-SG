...
- Konflikt wersji PyMuPDF podczas instalacji: PaddleOCR 2.7.0 wymaga `PyMuPDF<1.21.0`, dlatego w `requirements.txt` pinujemy zakres zgodny z tym ograniczeniem. Jeśli pip próbuje zainstalować nowszy wariant (np. z cache), odinstaluj obecny pakiet (`pip uninstall PyMuPDF -y`) i ponownie uruchom `pip install -r requirements.txt`.
- Konflikt podczas instalacji `pypdf`: w `requirements.txt` pinujemy `pypdf==3.12.0` (zależność nowsza niż `PyPDF2`). Jeśli pip zgłasza konflikt z istniejącą instalacją `PyPDF2`, odinstaluj ją poleceniem `pip uninstall PyPDF2 -y` i ponownie uruchom `pip install -r requirements.txt`.
